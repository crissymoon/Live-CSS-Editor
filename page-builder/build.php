...
@media (max-width: {$bp}px) {
  ...
  /*
   * SECTION INNER PADDING OVERRIDE (mobile)
   * Targets the inner centring <div> (not the section wrapper) so the
   * full-bleed background colour is preserved while content gets tight
   * 16 px gutters on both sides. The section wrapper itself has no
   * padding - all content spacing lives on the inner div with an inline
   * style generated by build.php. !important beats that inline style.
   */
  [data-pb-section] > div { padding-left: 16px !important; padding-right: 16px !important; }
