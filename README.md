...
--blocked dev-browser from god_funcs.py --will test seperately
...
--live-css/xcm_auth/smoke/MIGRATION_PLAN.md < Covers what must change in the live-css PHP app and the xcm_auth Go service when they run on different machines or containers. (Will go on GCLOUD VM --seperate from builder server)
    ...
    --------------------------------------------------
    Complexity                 92.8       90.0     reduce -2.8
    Scalability                82.5       85.0     need +2.5
    Dependencies              100.0       90.0   10.0 *
    Performance                96.0       80.0   16.0 *
    Technical Debt             40.0       40.0      0.0
    Memory                     89.5       85.0    4.5 *
    --------------------------------------------------
    Overall Score              87.4       81.1    6.3 *
    Risk Level                        HEALTHY (Low Risk)
