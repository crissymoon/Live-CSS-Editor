...
/**
 * DeepSeek Handler
 * DeepSeek's API is OpenAI-compatible. Accepts POST with a conversation
 * array and streams the response back using Server-Sent Events.
 *
 * POST body (JSON):
...
// DeepSeek uses /chat/completions under its base URL
...
curl_setopt_array($ch, [
    ...
    CURLOPT_WRITEFUNCTION  => function ($curl, $chunk) {
        ...
        foreach ($lines as $line) {
            ...
            // deepseek-reasoner surfaces reasoning_content separately
            $delta            = $event['choices'][0]['delta']     ?? [];
            $content          = $delta['content']                 ?? null;
            $reasoningContent = $delta['reasoning_content']       ?? null;
            if ($reasoningContent !== null && $reasoningContent !== '') {
                sseReasoning($reasoningContent);
            }
...
if ($httpCode >= 400) {
    sseError('DeepSeek API returned HTTP ' . $httpCode);
}
