...
_start_admin() {
    ...
    printf "\n"
    # show credentials
    local DB_PATH="$DIR/xcm_auth/xcm_auth_dev.db"
    local CRED_FILE="$DIR/xcm_auth/dev-credentials.json"
    box_top
    ...
    case "$CHOICE" in
        ...
        t|T)
            while true; do
                ...
                LINES_PY="$DIR/lines_count.py"
                SEC_PY="$DIR/security_ck.py"
