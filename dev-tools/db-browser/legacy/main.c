...
static void on_data_cell_activated(GtkTreeView *tree_view, GtkTreePath *path,
...
static void show_table_data(AppState *state, const char *table_name) {
    ...
    /* Remember which table we are browsing (used by cell editor) */
...
/* =========================================================================
 * Cell edit dialog -- double-click a cell in the Data Browser to edit it.
 * Uses the SQLite rowid to identify the row, then runs an UPDATE.
 * ========================================================================= */
static void on_data_cell_activated(GtkTreeView *tree_view, GtkTreePath *path,
                                   GtkTreeViewColumn *column, gpointer data)
{
    ...
    /* We need the rowid for this row. Query all rowids in the same order
       as the data browser (SELECT * FROM table LIMIT 100),
       then pick the one at the same row index. */
    ...
    /* Build the dialog with Cancel, Set NULL, and Save buttons */
    ...
    /* Scrolled text view for editing */
    GtkWidget *scroll = gtk_scrolled_window_new(NULL, NULL);
    gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(scroll),
                                   GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
    gtk_scrolled_window_set_shadow_type(GTK_SCROLLED_WINDOW(scroll), GTK_SHADOW_NONE);
...
static void populate_recent_combo(AppState *state) {
    ...
    if (!state->recent_paths) {
        ...
        for (GList *l = state->recent_paths; l != NULL; l = l->next) {
            ...
            /* Store path as object data -- the "text" property may not
               survive the popover auto-close that fires before "clicked". */
...
static void on_open_recent_db(GtkWidget *widget, gpointer data) {
    ...
    /* Retrieve the path from object data (set in populate_recent_combo).
       We cannot rely on the "text" property because the GtkPopover
       auto-closes before "clicked" fires, and the text may be gone. */
    ...
    /* Make a copy since the widget (and its data) may be destroyed
       when populate_recent_combo rebuilds the list during add_to_recent */
