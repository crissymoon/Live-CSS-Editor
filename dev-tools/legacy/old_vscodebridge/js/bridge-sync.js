...
(function (global) {
    ...
    var HTML_POLL_INTERVAL      = 3200;  // slightly offset to avoid request collision
    var JS_POLL_INTERVAL        = 3400;  // slightly offset to avoid request collision
    var PROJECT_POLL_INTERVAL    = 3600;  // poll for Copilot project saves
    ...
    // -------------------------------------------------------------------------
    // Apply HTML update from Copilot into the HTML editor
    // -------------------------------------------------------------------------
    function applyIncomingHTML(htmlText) {
        try {
            if (global.LiveCSS && global.LiveCSS.editor &&
                typeof global.LiveCSS.editor.setHtmlValue === 'function') {
                global.LiveCSS.editor.setHtmlValue(htmlText);
                console.warn('[BridgeSync] Copilot pushed HTML (' + htmlText.length + ' chars) into HTML editor');
            } else {
                warn('applyIncomingHTML: LiveCSS.editor.setHtmlValue not ready - editor may still be initializing');
                console.error('[BridgeSync] applyIncomingHTML: could not set HTML - LiveCSS.editor.setHtmlValue unavailable');
            }
        } catch (e) {
            err('applyIncomingHTML exception: ' + e.message);
        }
    }

    // -------------------------------------------------------------------------
    // Apply JS update from Copilot into the JS editor
    // -------------------------------------------------------------------------
    function applyIncomingJS(jsText) {
        try {
            if (global.LiveCSS && global.LiveCSS.editor &&
                typeof global.LiveCSS.editor.setJsValue === 'function') {
                global.LiveCSS.editor.setJsValue(jsText);
                console.warn('[BridgeSync] Copilot pushed JS (' + jsText.length + ' chars) into JS editor');
            } else {
                warn('applyIncomingJS: LiveCSS.editor.setJsValue not ready - editor may still be initializing');
                console.error('[BridgeSync] applyIncomingJS: could not set JS - LiveCSS.editor.setJsValue unavailable');
            }
        } catch (e) {
            err('applyIncomingJS exception: ' + e.message);
        }
    }

    // -------------------------------------------------------------------------
    // Poll bridge for HTML changes pushed by Copilot
    // -------------------------------------------------------------------------
    function pollHtmlChanges() {
        if (!_enabled) return;

        fetch(BRIDGE_URL + '?action=poll_html_changes', { method: 'GET', cache: 'no-store' })
        .then(function (res) {
            if (!res.ok) {
                return res.text().then(function (t) {
                    err('poll_html_changes HTTP ' + res.status + ': ' + t);
                });
            }
            return res.json().then(function (data) {
                if (!data) { err('poll_html_changes: empty response'); return; }
                if (!data.success) { warn('poll_html_changes: ' + (data.error || 'unknown')); return; }
                if (data.hasChanges && data.html != null) {
                    ackHtmlChange();
                    applyIncomingHTML(data.html);
                }
            });
        })
        .catch(function (e) {
            err('poll_html_changes network error: ' + e.message);
        });
    }

    // -------------------------------------------------------------------------
    // Poll bridge for JS changes pushed by Copilot
    // -------------------------------------------------------------------------
    function pollJsChanges() {
        if (!_enabled) return;

        fetch(BRIDGE_URL + '?action=poll_js_changes', { method: 'GET', cache: 'no-store' })
        .then(function (res) {
            if (!res.ok) {
                return res.text().then(function (t) {
                    err('poll_js_changes HTTP ' + res.status + ': ' + t);
                });
            }
            return res.json().then(function (data) {
                if (!data) { err('poll_js_changes: empty response'); return; }
                if (!data.success) { warn('poll_js_changes: ' + (data.error || 'unknown')); return; }
                if (data.hasChanges && data.js != null) {
                    ackJsChange();
                    applyIncomingJS(data.js);
                }
            });
        })
        .catch(function (e) {
            err('poll_js_changes network error: ' + e.message);
        });
    }

    // -------------------------------------------------------------------------
    // Acknowledge HTML change so next poll does not re-apply it
    // -------------------------------------------------------------------------
    function ackHtmlChange() {
        fetch(BRIDGE_URL + '?action=ack_html', {
            method:  'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        .then(function (data) {
            if (data && data.success) {
                log('HTML change acknowledged');
            } else {
                warn('ack_html unexpected response: ' + JSON.stringify(data));
            }
        })
        .catch(function (e) {
            err('ackHtmlChange network error: ' + e.message);
        });
    }

    // -------------------------------------------------------------------------
    // Acknowledge JS change so next poll does not re-apply it
    // -------------------------------------------------------------------------
    function ackJsChange() {
        fetch(BRIDGE_URL + '?action=ack_js', {
            method:  'POST',
            headers: { 'Content-Type': 'application/json' },
            body:    JSON.stringify({ acked: true }),
        })
        ...
        .then(function (data) {
            if (data && data.success) {
                log('JS change acknowledged');
            } else {
                warn('ack_js unexpected response: ' + JSON.stringify(data));
            }
        })
        .catch(function (e) {
            err('ackJsChange network error: ' + e.message);
        });
    }
    ...
    function pollProjectSave() {
        if (!_enabled) return;
        if (!global.LiveCSS || !global.LiveCSS.storage || typeof global.LiveCSS.storage.pollProjectUpdate !== 'function') {
            return; // storage not loaded yet
        }
        ...
        global.LiveCSS.storage.pollProjectUpdate()
            .then(function (data) {
                ...
                console.warn('[BridgeSync] Copilot saved project "' + name + '" (source: ' + source + ') -- click Load in the app to apply it to all editors');

                // Acknowledge so we don't fire again
                global.LiveCSS.storage.ackProjectUpdate()
                    .catch(function (e) { err('ackProjectUpdate failed: ' + e.message); });

                // Auto-load into editors if the project was saved by copilot
                if (source === 'vscode-copilot') {
                    global.LiveCSS.storage.loadDbProject(name)
                        .then(function (project) {
                            if (!project) {
                                err('pollProjectSave: loadDbProject returned null for "' + name + '"');
                                return;
                            }
                            try {
                                if (global.LiveCSS.editor) {
                                    if (typeof global.LiveCSS.editor.setHtmlValue === 'function') {
                                        global.LiveCSS.editor.setHtmlValue(project.html || '');
                                    }
                                    if (typeof global.LiveCSS.editor.setCssValue === 'function') {
                                        global.LiveCSS.editor.setCssValue(project.css || '');
                                    }
                                    if (typeof global.LiveCSS.editor.setJsValue === 'function') {
                                        global.LiveCSS.editor.setJsValue(project.js || '');
                                    }
                                    console.warn('[BridgeSync] Auto-loaded project "' + name + '" into all 3 editors');
                                    document.dispatchEvent(new CustomEvent('bridgeProjectLoaded', {
                                        detail: { name: name, source: source }
                                    }));
                                } else {
                                    warn('pollProjectSave: LiveCSS.editor not ready, project saved but not auto-loaded');
                                }
                            } catch (e) {
                                err('pollProjectSave auto-load exception: ' + e.message);
                            }
                        })
                        .catch(function (e) {
                            err('pollProjectSave loadDbProject error: ' + e.message);
                        });
                }
            })
            .catch(function (e) {
    ...
    function startIntervals() {
        ...
        _htmlTimer      = setInterval(pollHtmlChanges,       HTML_POLL_INTERVAL);
        _jsTimer        = setInterval(pollJsChanges,         JS_POLL_INTERVAL);
        _projectTimer   = setInterval(pollProjectSave,       PROJECT_POLL_INTERVAL);
        log('Intervals started (push=' + PUSH_INTERVAL_MS + 'ms, poll=' + POLL_INTERVAL_MS + 'ms, refresh=' + REFRESH_INTERVAL_MS + 'ms, wireframe=' + WIREFRAME_POLL_INTERVAL + 'ms, html=' + HTML_POLL_INTERVAL + 'ms, js=' + JS_POLL_INTERVAL + 'ms, project=' + PROJECT_POLL_INTERVAL + 'ms)');
    }
    ...
    function stopIntervals() {
        ...
        clearInterval(_htmlTimer);
        clearInterval(_jsTimer);
        clearInterval(_projectTimer);
        clearTimeout(_pushBounce);
        _pushTimer = _pollTimer = _refreshTimer = _wireframeTimer = _htmlTimer = _jsTimer = _projectTimer = _pushBounce = null;
    ...
    var BridgeSync = {
        ...
        poll: function () { pollChanges(); pollWireframeChanges(); pollHtmlChanges(); pollJsChanges(); },
